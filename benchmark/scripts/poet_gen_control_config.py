...
def ratio_for_id(state_id):
    df_heartbeats = pd.read_csv(
        f"../experiments/cpu_states/heartbeat_{state_id}.log", delimiter=" ", skipinitialspace=True, index_col="Beat")
...
import glob
num_states = len(glob.glob("../experiments/cpu_states/heartbeat_*.log"))
...
for cores, a in areas.items():
    ...
    ax1.text((a[0]+a[1])/2, sum(ax1.get_ylim())*0.7, f"{int(cores)+1} Core(s)",
             color=f"C{int(cores)+2}", horizontalalignment="center", fontsize=10)

ax1.set_xlabel('State Number')
ax1.set_ylabel('Speedup / Powerup')
ax1.legend()
